...
try {
    ...
    Write-Host "[OK] .NET CLI version: $dotnetVersion" -ForegroundColor Green
} catch {
...
if (-not $efToolsInstalled) {
    Write-Host "[WARNING] EF Core tools not found. Installing..." -ForegroundColor Yellow
    dotnet tool install --global dotnet-ef
    Write-Host "[OK] EF Core tools installed successfully" -ForegroundColor Green
} else {
    Write-Host "[OK] EF Core tools are already installed" -ForegroundColor Green
}
...
try {
    ...
    if ($DropDatabase) {
        ...
        try {
            ...
            Write-Host "[WARNING] Database does not exist or could not be dropped" -ForegroundColor Yellow
        }
    }
    ...
    if (-not $SkipSeeding) {
        ...
        dotnet run --no-build --configuration Release -- seed
        
        if ($LASTEXITCODE -eq 0) {
            Write-Host "[OK] Database seeded successfully" -ForegroundColor Green
        } else {
            Write-Host "[WARNING] Seeding completed. Check output above for any issues." -ForegroundColor Yellow
        }
    } else {
    ...
    Write-Host ""
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host "Database Setup Completed[WARNING]" -ForegroundColor Green
    Write-Host "========================================" -ForegroundColor Cyan
    Write-Host ""
